...
export default {
  ...
  methods: {
    drawWordCloud() {
      this.$axios
        .get("/api/acbi/getWordCloudData")
        .then(({ data }) => {
          if (data.success) {
            const wordCloudData = JSON.parse(
              JSON.stringify(data.data)
                .replace(/itemName/g, "name")
                .replace(/id/g, "value")
            ).map(item =>
              Object.assign({}, { value: item.value, name: item.name })
            );
            ...
            this.$message.warning("word cloud load fail！");
          }
        })
        .catch(err => {
          console.log(err);
          this.$message.error("error!! word cloud load fail!!!");
        });
    }
  },
  computed: {
    chartWordCloud() {
      return this.$echarts.init(this.$util.getDom("wordCloud"));
    }
  }
};
</script>
